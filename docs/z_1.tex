...
En geometría, una \textbf{traslación} es una isometría en el espacio euclídeo caracterizada por un vector, tal que, a cada punto \emph{P} de un objeto o figura se le hace corresponder otro punto \emph{P'} , tal que: {[}\{\textbackslash displaystyle \{\textbackslash begin\{cases\}T\_\{\textbackslash vec \{u\}\}:\textbackslash mathbb \{R\} \^{}\{n\}\textbackslash to \textbackslash mathbb \{R\} \^{}\{n\}\&\{\textbackslash overrightarrow \{PP'\}\}=\{\textbackslash vec \{u\}\}\textbackslash\textbackslash P\textbackslash mapsto P'=T(P)=P+\{\textbackslash vec \{u\}\}\textbackslash end\{cases\}\}\}{]}
...
\textbf{Rotación} es el movimiento de cambio de orientación de un cuerpo o un sistema de referencia de forma que una línea (llamada \emph{eje de rotación}) o un punto permanece fijo.
...
En topología, un \textbf{homeomorfismo} es una función de un espacio topológico a otro, que cumple con ser una función biyectiva continua y cuya inversa es continua. En este caso, los dos espacios topológicos se dicen \textbf{homeomorfos}.
...
En matemáticas, un \textbf{isomorfismo} (del griego iso-morfos: Igual forma) es un homomorfismo (o más generalmente un morfismo) que admite un inverso. \hspace{0pt} El concepto matemático de \textbf{isomorfismo} pretende captar la idea de tener la misma estructura.
